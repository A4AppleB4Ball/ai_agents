...
Converts internal `AMessage` to a protocol directly consumable by the frontend:
1. WebSocket real-time events (conversation_event).
2. History snapshot messages (Message[]).
"""
...
class ProtocolAdapter:
    ...
    This class is the sole exit point for backend message protocols, responsible for
    eliminating SDK raw structure differences and generating stable frontend message
    ...
    def build_ws_event(self, message: AMessage) -> Optional[AEvent]:
        """Convert internal message to a WebSocket conversation_event."""
    ...
    def build_history_messages(self, messages: List[AMessage]) -> List[Dict[str, Any]]:
        """Convert history messages to stable snapshot list."""
    ...
    def _apply_snapshot(self, history: List[Dict[str, Any]], snapshot: Dict[str, Any]) -> None:
        """Apply snapshot to history list (upsert + tool_result merge)."""
    ...
    def _merge_tool_result_snapshot(self, history: List[Dict[str, Any]], tool_result: Dict[str, Any]) -> bool:
        """Merge tool_result into corresponding assistant message."""
    ...
    def _convert_message_snapshot(self, message: AMessage) -> Optional[Dict[str, Any]]:
        """Convert internal message to frontend snapshot message."""
    ...
    def _convert_stream_delta(self, message: AMessage) -> Optional[Dict[str, Any]]:
        """Convert internal stream message to frontend delta."""
        payload = self._to_dict(message.message)
    ...
    def _resolve_target_message_id(self, session_key: str, blocks: List[Dict[str, Any]]) -> Optional[str]:
        """Reverse-lookup assistant message_id by tool_use_id."""
    ...
    def _index_tool_use_owner(self, session_key: str, message_id: str, blocks: List[Dict[str, Any]]) -> None:
        """Build index from tool_use_id to assistant message_id."""
    ...
    def _next_seq(self, session_key: str) -> int:
        """Get and increment event sequence number within session."""
    ...
    def _resolve_agent_id(self, session_key: str, fallback: str) -> str:
        """Return the session routing key used by the frontend."""
    ...
    def _merge_assistant_content(existing: Any, incoming: Any) -> List[Dict[str, Any]]:
        """Merge assistant content blocks, ensuring idempotency."""
    ...
    def _content_block_key(block: Dict[str, Any]) -> Optional[str]:
        """Generate idempotent key for a content block."""
    ...
    def _to_dict(value: Any) -> Dict[str, Any]:
        """Convert an object to a dict where possible."""
        if value is None:
            return {}
