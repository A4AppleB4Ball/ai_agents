...
class BaseFile(AModel):
    ...
    file_bnid: Union[int, str] = Field(..., description="File business ID")
    deploy_id: Union[int, str] = Field(default="", description="Deployment ID")
    user_bnid: Union[int, str] = Field(default="", description="User database ID")
    cache_path: str = Field(default="", description="Local cache path", exclude=True)
...
class OSSFile(BaseFile):
    ...
    file_bucket: str = Field(..., description="File bucket")
    oss: Literal["minio", "tencent_cos", "local"] = Field(settings.DEFAULT_OSS_CLIENT, description="OSS type")
