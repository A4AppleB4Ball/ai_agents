/**
 * useAgentSession Hook - Main entry
 *
 * Manages the WebSocket connection, message handling, and session state for Agent sessions
 */
  ...
  if (event.type === 'content_block_delta' && event.delta && typeof event.index === 'number') {
    ...
    if (block.type === 'tool_use' && delta.type === 'input_json_delta') {
      try {
        ...
        // Ignore incomplete JSON and wait for subsequent deltas
      }
      return updatedMessage;
    }
  }
...
export function useAgentSession(options: UseAgentSessionOptions = {}): UseAgentSessionReturn {
  ...
  // sessionKey is initially null and is set only when creating or loading a session
  const [sessionKey, setstring] = useState<string | null>(null);
  // Permission request state - supports multiple concurrent requests keyed by request_id.
  // tool_use_id binds a request to the exact tool_use block, which is required when the
  // model issues several tool calls of the same name in one turn.
  ...
  const handleWebSocketMessage = useCallback((backendMsg: any) => {
    ...
    if (backendMsg.event_type) {
      ...
      if (backendMsg.event_type === 'permission_request') {
        ...
        if (!data.tool_use_id) {
          throw new Error(
            `permission_request missing tool_use_id (request_id=${data.request_id})`
          );
        }
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    if (!sessionKey) {
      const errorMsg = 'Please select or create a session first';
    ...
    if (wsState !== 'connected') {
      const errorMsg = 'WebSocket is not connected, please try again shortly';
      console.error('[sendMessage] WebSocket not connected, state:', wsState);
      setError(errorMsg);
      return;
    }

    console.debug('[sendMessage] Sending message, sessionKey:', sessionKey);
    ...
    try {
      ...
      // Send to the backend with round_id to keep the frontend and backend consistent
      wsSend({
        type: 'chat',
        content,
      ...
      console.debug('[sendMessage] Message sent successfully');
    } catch (err) {
  ...
  const stopGeneration = useCallback(() => {
    ...
    console.debug('[useAgentSession] stopGeneration called:', {
      sessionKey,
    ...
    if (sessionKey && wsSend) {
      ...
      console.debug('[useAgentSession] Sending stop message:', interruptMsg);
      console.debug('[useAgentSession] WebSocket state:', wsState);

      try {
        wsSend(interruptMsg);
        console.debug('[useAgentSession] Stop message sent');
      } catch (error) {
        console.error('[useAgentSession] Failed to send stop message:', error);
      }
    } else {
      console.warn('[useAgentSession] Unable to send stop message:', {
  ...
  /**
   * Send permission response for a specific request (also used for AskUserQuestion)
   */
  const sendPermissionResponse = useCallback((requestId: string, decision: 'allow' | 'deny', userAnswers?: UserQuestionAnswer[]) => {
  ...
  }, [pendingPermissions, sessionKey, wsSend]);
  ...
  const deleteRound = useCallback(async (roundId: string) => {
    ...
    try {
      ...
      console.debug('[deleteRound] Deleted successfully:', roundId);
    } catch (err) {
      console.error('[deleteRound] Delete failed:', err);
      setError(err instanceof Error ? err.message : 'Failed to delete round');
    }
  }, [sessionKey]);

  /**
   * Regenerate the last round response
   * Keep the user's question, delete only the response, and regenerate
   */
  const regenerate = useCallback(async (roundId: string) => {
    // Use ref to get the latest messages
    if (!sessionKey) {
      console.error('[regenerate] No sessionKey or messages');
      return;
    }

    // Find the user message from the last round
    const lastUserMessage = messages.findLast(m => m.role === 'user' && m.message_id === roundId);
    console.debug('[regenerate] Found the last round user message:', lastUserMessage);
    ...
    try {
      // 1. Delete the full round data from the backend
      await deleteRound(roundId);

      // 2. Send the message
      await sendMessage(lastContent);

      console.debug('[regenerate] Regenerated successfully while keeping the user question');
    } catch (err) {
      console.error('[regenerate] Failed to regenerate:', err);
